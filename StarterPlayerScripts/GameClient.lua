...
local function createGUI()
	...
	menuFrame.Position = UDim2.new(0.2, 0, 0.075, 0)
	...
	roomsList.Size = UDim2.new(1, -40, 1, -160)
	roomsList.Position = UDim2.new(0, 20, 0, 90)
	...
	playButton.Position = UDim2.new(0.3, 0, 1, -60)
	...
	gameOverFrame.Size = UDim2.new(0.6, 0, 0.85, 0)
	gameOverFrame.Position = UDim2.new(0.2, 0, 0.075, 0)
	...
	scoresFrame.Size = UDim2.new(1, -40, 1, -130)
	scoresFrame.Position = UDim2.new(0, 20, 0, 60)
	...
	replayButton.Position = UDim2.new(0.3, 0, 1, -60)
...
StartGameEvent.OnClientEvent:Connect(function(role, seekers, hiders)
	...
	if gui then
		...
		if hideAndSeekGUI then
			local menuFrame = hideAndSeekGUI:FindFirstChild("MenuFrame", true)
			local gameFrame = hideAndSeekGUI:FindFirstChild("GameFrame", true)
...
EndGameEvent.OnClientEvent:Connect(function(seekers, hiders, playersData)
	...
	if gui then
		...
		if hideAndSeekGUI then
			local gameFrame = hideAndSeekGUI:FindFirstChild("GameFrame", true)
			local gameOverFrame = hideAndSeekGUI:FindFirstChild("GameOverFrame", true)

			if gameFrame then gameFrame.Visible = false end
			if gameOverFrame then gameOverFrame.Visible = true end

			-- Afficher les scores
			local scoresFrame = gameOverFrame and gameOverFrame:FindFirstChild("ScoresFrame", true)
			if scoresFrame then
...
SetGamePhaseEvent.OnClientEvent:Connect(function(phase, timeRemaining)
	...
	if gui then
		local hideAndSeekGUI = gui:FindFirstChild("HideAndSeekGUI")
		if hideAndSeekGUI then
			local phaseLabel = hideAndSeekGUI:FindFirstChild("PhaseLabel", true)
			local timerLabel = hideAndSeekGUI:FindFirstChild("TimerLabel", true)
			local roleLabel = hideAndSeekGUI:FindFirstChild("RoleLabel", true)
			local instructionsLabel = hideAndSeekGUI:FindFirstChild("InstructionsLabel", true)
			local camouflageButton = hideAndSeekGUI:FindFirstChild("CamouflageButton", true)
...
local function setupButtons()
	...
	local playButton = hideAndSeekGUI:FindFirstChild("PlayButton", true)
	if playButton then
	...
	local backButton = hideAndSeekGUI:FindFirstChild("BackButton", true)
	if backButton then
	...
	local replayButton = hideAndSeekGUI:FindFirstChild("ReplayButton", true)
